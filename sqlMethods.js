module.exports = {
	...
	newUser: function(id, fname, lname, category, description, img, username, pword, email, phone) {return `INSERT INTO MENTORS(ID, FNAME, LNAME, CATEGORY, DESCRIPTION, IMG) VALUES(${id}, "${fname}", "${lname}", "${category}", "${description}", "${img}"); INSERT INTO MENTORDATA(ID, USERNAME, PWORD, EMAIL, PHONE) VALUES("${id}", "${username}", "${pword}", "${email}", "${phone}")`},

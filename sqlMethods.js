module.exports = {
	...
	loginCheck: function(username, pword) {return `SELECT ID FROM MENTORDATA WHERE USERNAME = "${username}" AND PWORD = "${pword}";`},
	//Generic edit
	...
	newUser1: function(id, fname, lname, category, description, img, username, pword, email, phone) {return `INSERT INTO MENTORS(ID, FNAME, LNAME, CATEGORY, DESCRIPTION, IMG) VALUES(${id}, "${fname}", "${lname}", "${category}", "${description}", "${img}");`}, 
	newUser2: function(id, username, pword, email, phone) {return `INSERT INTO MENTORDATA(ID, USERNAME, PWORD, EMAIL, PHONE) VALUES(${id}, "${username}", "${pword}", "${email}", "${phone}");`},
	newUser3: function(id) {return `INSERT INTO SITEDATA(ID) VALUES(${id});`},
